		...
		<u-list @scrolltolower="scrolltolower" :height="scollListHeight">
			...
			<u-list-item class="bg-white rounded">
				...
				<view class="margin-center pb-4">
					<text class="font-sm text-muted">版权与免责声明系为保证本网站的正常发展、规避意外风险而设，其初衷是为了向用户提供不间断的优质服务。因此，建议您在接受本站服务之前，请务必仔细阅读本条款</text>
				</view>
			</u-list-item>
		</u-list>
				...
				return new Promise((resolve, reject) => {
					this.$nextTick(function() {
						...
						query1.select('.InfoHeight').boundingClientRect(data => {
							...
							// #endif
							let lineNum = Math.floor((height + Math.floor((lineHeight - 14)/2)) / lineHeight)
